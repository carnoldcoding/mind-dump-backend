...
/**
 * Removes everything matching a filter, object and record together.
 *
 * One at a time through `remove` rather than a bulk delete, so a batch cannot
 * clear rows whose objects are still in the bucket: the sequence that keeps
 * those two in step is the whole reason this module exists, and a `deleteMany`
 * beside a loop of `DeleteObjectCommand`s would quietly stop honouring it.
 *
 * A failure part-way leaves the rest in place. That is recoverable — the
 * caller can try again, and audit tooling can see what is left — where a
 * half-finished delete that had already dropped its owner would not be.
 */
async function removeWhere(kind, filter) {
  const { collection } = kindConfig(kind);
  const docs = await getDB().collection(collection).find(filter, { projection: { _id: 1 } }).toArray();

  const removed = [];
  for (const doc of docs) removed.push(await remove(kind, doc._id));
  return removed;
}

module.exports = { upload, list, remove, removeWhere };
